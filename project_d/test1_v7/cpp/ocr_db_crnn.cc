...
void RunRecModel(std::vector<std::vector<std::vector<int>>> boxes, cv::Mat img,
  ...
  for (int i = boxes.size() - 1; i >= 0; i--) {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "GetRotateCropImage");
    float wh_ratio =
        static_cast<float>(crop_img.cols) / static_cast<float>(crop_img.rows);

    resize_img = CrnnResizeImg(crop_img, wh_ratio, rec_image_height);
    resize_img.convertTo(resize_img, CV_32FC3, 1 / 255.f);
    __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "CrnnResizeImg");
    const float *dimg = reinterpret_cast<const float *>(resize_img.data);
    ...
    NeonMeanScale(dimg, data0, resize_img.rows * resize_img.cols, mean, scale);
    __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "NeonMeanScale");
    auto preprocess_end = std::chrono::steady_clock::now();
    //// Run CRNN predictor
    auto inference_start = std::chrono::steady_clock::now();
    predictor_crnn->Run();
    __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "predictor_crnn->Run();");
    // Get output and run postprocess
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "Get output and run postprocess");
    // ctc decode
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "for_b");
    for (int n = 0; n < predict_shape[1]; n++) {
      argmax_idx = int(Argmax(&predict_batch[n * predict_shape[2]],
                              &predict_batch[(n + 1) * predict_shape[2]]));
      __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "argmax_idx");
      max_value =
          float(*std::max_element(&predict_batch[n * predict_shape[2]],
                                  &predict_batch[(n + 1) * predict_shape[2]]));
      __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "if_b");
      if (argmax_idx > 0 && (!(n > 0 && argmax_idx == last_index))) {
        score += max_value;
        __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "score");
        count += 1;
        __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "count");
        __android_log_print(ANDROID_LOG_DEBUG, "if", "%s", charactor_dict[argmax_idx].c_str());
        __android_log_print(ANDROID_LOG_DEBUG, "if", "%s", str_res.c_str());
        str_res += charactor_dict[argmax_idx];
        __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "str_res");
      }
      __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "if_a");
      last_index = argmax_idx;
    }
    __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "for_a");
    score /= count;
    rec_text.push_back(str_res);
    __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "rec_text.push_back(str_res)");
    rec_text_score.push_back(score);
    __android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "rec_text_score.push_back(score);");
...
__android_log_print(ANDROID_LOG_DEBUG, "RunRecModel", "%s", "res");
}
...
std::vector<std::pair<std::string, double>> system(std::vector<cv::Mat> &cv_all_imgs,const string tool) {
  ...
  auto Config = LoadConfigTxt(det_config_path);
   __android_log_print(ANDROID_LOG_DEBUG, "CS", "%s", "det_config");
  ...
  __android_log_print(ANDROID_LOG_DEBUG, "system", "%s", "det_model");
  auto rec_predictor = loadModel(rec_model_file, std::stoi(num_threads));
  __android_log_print(ANDROID_LOG_DEBUG, "system", "%s", "rec_model");
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "system", "%s", "det_model_run");
    std::vector<std::string> rec_text;
    std::vector<float> rec_text_score;
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "system", "%s", "RunRecModel");
    //// visualization
  ...
  __android_log_print(ANDROID_LOG_DEBUG, "system", "%s", "Res");
  return results;
...
std::string cap2str(cv::Mat cap, const string tool_path) {
  //__android_log_print(ANDROID_LOG_DEBUG, "cap2str", "%s", "two_valuing_b");
  cv::Mat binary = two_valuing(cap, thresh_O); 
  //__android_log_print(ANDROID_LOG_DEBUG, "cap2str", "%s", "two_valuing_a");
  cv::Mat img1, img2;
  //__android_log_print(ANDROID_LOG_DEBUG, "cap2str", "%s", "get_spin_angle_b");
  double rotate_angle = get_spin_angle(binary);
  //__android_log_print(ANDROID_LOG_DEBUG, "cap2str", "%s", "get_spin_angle_a");
  rotateImage(cap, img1, rotate_angle, 0);
  rotateImage(cap, img2, rotate_angle + 180, 0);
  //__android_log_print(ANDROID_LOG_DEBUG, "cap2str", "%s", "rotateImage_a");
  std::vector<cv::Mat> cv_all_imgs;
  cv_all_imgs.push_back(img1);
  cv_all_imgs.push_back(img2);


  std::vector<std::pair<std::string, double>> outputs = system(cv_all_imgs, tool_path);
  //__android_log_print(ANDROID_LOG_DEBUG, "cap2str", "%s", "system_a");
  ...
  //__android_log_print(ANDROID_LOG_DEBUG, "cap2str", "%s", "return_a");
  return ret;
}
