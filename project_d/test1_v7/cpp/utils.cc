...
const float scalingfactor = -89.400604;
...
vector<MyData> circle_detector(const string path_one_img,const string tool_path) {
    Mat img_left = imread(path_one_img, 1);

    vector<Vec3f> circles_left;
    circles_left = circle_help(img_left);
    //__android_log_print(ANDROID_LOG_DEBUG, "circle_detector", "%s", "cirle_help");
    ...
    if (!circles_left.empty()) {
        for (size_t i = 0; i < circles_left.size(); i++) {
           ...
           //__android_log_print(ANDROID_LOG_DEBUG, "circle_detector", "%s", "cap2str_bef");
            string cur_string;
            try{
                cur_string = cap2str(cropped, tool_path);
            }
            //__android_log_print(ANDROID_LOG_DEBUG, "circle_detector", "%s", "cap2str_aft");
            catch(...){
            ...
            if (cur_string.size() == 0){
            	__android_log_print(ANDROID_LOG_DEBUG, "circle_detector", "%s", "cur_string.size() == 0");
                continue;
            }
            else{
